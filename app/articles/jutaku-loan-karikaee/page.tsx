...
export const metadata: Metadata = {
  ...
  description:
    "住宅ローンの借り換えで本当に得をする条件と損益分岐点の計算方法を解説。2026年の金利環境で借り換えを検討すべき人・やめておくべき人の判断基準と、借り換え後の総返済額シミュレーションも紹介。",
  keywords: [
    "住宅ローン 借り換え",
    "住宅ローン 借り換え メリット",
    "借り換え 損益分岐点",
  ...
  openGraph: {
    ...
    description: "借り換えの損益分岐点の計算方法と、2026年に借り換えを検討すべき人の判断基準。総返済額シミュレーション付き。",
  },
};
    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">住宅ローン借り換え</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">借り換え・住宅ローン</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          住宅ローンの<span className="text-blue-400">借り換えで得する人・損する人</span><br />
          【損益分岐点を計算】
        </h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">ローン残高</th>
                  <th className="text-center px-4 py-3 font-bold text-blue-300 border-b border-slate-700">金利差0.3%</th>
                  <th className="text-center px-4 py-3 font-bold text-emerald-400 border-b border-slate-700">金利差0.5%</th>
                  <th className="text-center px-4 py-3 font-bold text-orange-400 border-b border-slate-700">金利差1.0%</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-200">{row.zanko}</td>
                    <td className="px-4 py-3 text-center text-xs text-blue-300">{row.p3}</td>
                    <td className="px-4 py-3 text-center text-xs text-emerald-400">{row.p5}</td>
                    <td className="px-4 py-3 text-center text-xs text-orange-400">{row.p10}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-500">※概算値。実際の削減額は残存期間・元利均等の計算によって異なります。</p>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20 mb-4">
            <p className="text-sm font-bold text-blue-200 mb-3">損益分岐点の計算式</p>
            <div className="bg-slate-800 rounded-lg p-3 text-center mb-3">
              <p className="text-base font-black text-blue-300">回収月数 = 諸費用合計 ÷ 月々の削減額</p>
            </div>
            <p className="text-xs text-slate-300">例：諸費用60万円、月削減額1万円 → 60ヶ月（5年）で回収</p>
          </div>
          <div className="space-y-3 mb-4">
            <p className="text-sm font-bold text-slate-100">主な借り換え諸費用の目安</p>
            {[
              ...
              <div key={i} className="bg-slate-800 rounded-xl p-3 border border-slate-700 flex justify-between items-center">
                <span className="text-xs text-slate-200">{row.item}</span>
                <span className="text-xs font-bold text-white">{row.cost}</span>
              </div>
            ))}
          </div>
          <div className="bg-yellow-500/10 border border-yellow-200 rounded-xl p-4">
            <p className="text-xs font-bold text-yellow-300 mb-1">⚠️ 目安：残存期間が10年未満なら効果は薄い</p>
            <p className="text-xs text-slate-300">諸費用を回収するには最低でも3〜5年の期間が必要です。残存期間が短い場合は損益分岐点を超える前に完済してしまうケースがあります。</p>
          </div>
        </section>

        {/* ━━ モゲチェックCTA ━━ */}
        <div className="bg-slate-900 border border-blue-500/25 rounded-2xl p-5 mb-10">
          <p className="text-xs font-bold text-blue-400 mb-1">💡 借り換え先の金利、複数行を一括比較</p>
          <p className="text-sm font-black text-white mb-2">「モゲチェック」で最適な借り換え先を無料診断</p>
          <p className="text-xs text-slate-400 mb-3">現在のローン残高・金利を入力するだけで、借り換え効果と最適な金融機関を提案。審査通過率も確認できます。</p>
          <a
        ...
        <div className="border-2 border-amber-400 bg-amber-50 rounded-2xl p-5 mb-6">
          <p className="text-sm font-black text-amber-800 mb-1">💡 借り換えを検討中なら、まず今の金利との差を確認しましょう</p>
          <p className="text-xs text-amber-700 mb-3">最新の住宅ローン金利を複数行で比較。今より0.3%以上低い銀行があれば借り換えの検討価値があります。</p>
          <a
            href="https://px.a8.net/svt/ejp?a8mat=4AZGC3+F9J44Y+3SUE+15RCDE"
            target="_blank"
        ...
        <section>
          ...
          <div className="space-y-2">
            <Link href="/articles/jutaku-loan-hendokinri-koteikinri" className="flex items-center gap-3 bg-slate-800 rounded-xl p-3 border border-slate-700 hover:border-blue-500/40 transition-colors group">
              ...
              <span className="text-sm text-slate-200 group-hover:text-blue-400">住宅ローンは変動金利と固定金利どっちがいい？2025年の選び方</span>
            </Link>
