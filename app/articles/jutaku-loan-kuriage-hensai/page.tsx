    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">繰り上げ返済</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">繰り上げ返済</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          住宅ローンの<span className="text-blue-400">繰り上げ返済</span>はすべき？<br />
          メリット・デメリットを解説
        </h1>
          ...
          <div className="grid grid-cols-1 gap-4 mb-4">
            {[
              ...
              <div key={i} className={`rounded-xl p-4 border ${item.color}`}>
                <p className="text-sm font-black text-white mb-2">{item.emoji} {item.type}</p>
                <p className="text-xs text-slate-200 mb-2">{item.desc}</p>
                <p className="text-xs mb-1"><span className="font-bold text-emerald-400">✅ メリット：</span><span className="text-slate-200">{item.merit}</span></p>
                <p className="text-xs"><span className="font-bold text-orange-400">⚠️ デメリット：</span><span className="text-slate-200">{item.demerit}</span></p>
              </div>
            ))}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">繰り上げ方法</th>
                  <th className="text-right px-4 py-3 font-bold text-blue-300 border-b border-slate-700">利息削減額</th>
                  <th className="text-right px-4 py-3 font-bold text-emerald-400 border-b border-slate-700">期間短縮</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-slate-100">{row.method}</td>
                    <td className="px-4 py-3 text-right text-blue-300 font-bold">{row.save}</td>
                    <td className="px-4 py-3 text-right text-emerald-400 text-xs">{row.period}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-500">※金利1.0%・残25年・残高3,000万円で試算。実際の効果は借入条件により異なります。</p>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="bg-yellow-500/10 rounded-xl p-5 border border-yellow-200 mb-4">
            <p className="text-sm font-bold text-yellow-300 mb-3">⚠️ 繰り上げ返済で「控除メリット」が減る場合がある</p>
        ...
        <section className="mb-10">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-700 mb-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">住宅ローン金利</th>
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">推奨する行動</th>
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">理由</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 font-bold text-white">{row.rate}</td>
                    <td className="px-4 py-3 text-blue-300 font-semibold text-xs">{row.action}</td>
                    <td className="px-4 py-3 text-slate-300 text-xs">{row.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-500">※あくまで考え方の目安。個人の状況・リスク許容度・控除期間の有無により最適解は変わります。</p>
        </section>
