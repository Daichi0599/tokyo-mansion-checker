    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">資産価値・売却</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">資産価値・売却</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          <span className="text-blue-400">資産価値が落ちにくい</span>マンションの条件とは？<br />
          立地・築年数で解説
        </h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  ...
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">新築比（一般エリア）</th>
                  <th className="text-right px-4 py-3 font-bold text-blue-300 border-b border-slate-700">新築比（駅近・都心）</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 font-bold text-white">{row.years}</td>
                    <td className="px-4 py-3 text-right text-slate-200">{row.general}</td>
                    <td className="px-4 py-3 text-right text-blue-300 font-semibold">{row.prime}</td>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                <p className="text-sm font-bold text-red-400 mb-1">❌ {item.point}</p>
                <p className="text-xs text-slate-300 leading-relaxed">{item.detail}</p>
              </div>
            ))}
          </div>
        ...
        <section>
          ...
          <div className="space-y-2">
            ...
            <Link href="/articles/mansion-kattewa-ikenai-joken" className="flex items-center gap-3 bg-slate-800 rounded-xl p-3 border border-slate-700 hover:border-blue-500/40 transition-colors group">
              <span className="text-xl">🚩</span>
              <span className="text-sm text-slate-200 group-hover:text-blue-400">買ってはいけないマンションの特徴10選【立地・建物・管理で後悔しない】</span>
            </Link>
          </div>
        </section>
