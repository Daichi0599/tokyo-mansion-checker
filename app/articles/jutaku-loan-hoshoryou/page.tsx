    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">保証料・融資手数料</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">諸費用・住宅ローン</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          住宅ローンの<span className="text-blue-400">保証料と融資手数料</span>、<br />
          どちらが得？違いを解説
        </h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">方式</th>
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">支払い方法</th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">3,000万・35年の目安</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-xs font-semibold text-white">{row.method}</td>
                    <td className="px-4 py-3 text-xs text-slate-300">{row.detail}</td>
                    <td className="px-4 py-3 text-right text-xs font-bold text-blue-300">{row.cost}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mb-10">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-700 mb-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">パターン</th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">初期費用</th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">金利コスト増分</th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">概算総コスト</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    ...
                    <td className="px-4 py-3 text-right text-xs text-slate-200">{row.init}</td>
                    <td className="px-4 py-3 text-right text-xs text-slate-200">{row.rate}</td>
                    <td className="px-4 py-3 text-right text-xs font-bold text-blue-300">{row.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-500">※融資手数料は金融機関により「借入額の2.2%」と「定額（数万円）」に分かれます。ネット銀行は定額・低額型が多い傾向。</p>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20 mb-6">
            <p className="text-sm font-bold text-blue-200 mb-2">保証料の返戻（例）</p>
          ...
          <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
            <p className="text-sm font-bold text-slate-100 mb-3">どちらを選ぶべき？判断フロー</p>
            <div className="space-y-3">
              {[
                { q: "繰り上げ返済を積極的に行う予定がある？", a: "→ 外枠方式（一括払い）が有利", color: "text-blue-300" },
                { q: "諸費用の現金をなるべく抑えたい？", a: "→ 内枠方式（金利上乗せ）を検討", color: "text-emerald-400" },
                { q: "ネット銀行を検討している？", a: "→ 多くが定額手数料型のため保証料不要のケースも", color: "text-purple-400" },
                { q: "10年以内の完済を予定している？", a: "→ 外枠・一括払いで返戻を受けた方がお得な場合が多い", color: "text-blue-300" },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-2">
                  <span className="text-slate-500 text-xs mt-0.5 font-bold">Q{i + 1}.</span>
                  <div>
                    <p className="text-xs font-semibold text-slate-100">{item.q}</p>
