...
export const metadata: Metadata = {
  ...
  description:
    "マンション購入の流れを「予算決め→物件探し→申込→審査→契約→引渡し」の全ステップで解説。初めての購入でも迷わないよう、各ステップの注意点・かかる期間・諸費用の目安も網羅。まず無料診断で予算を確認しましょう。",
  keywords: [
    "マンション 購入 流れ",
    "マンション 買い方 手順",
    "マンション 購入 初めて",
  ...
  openGraph: {
    title: "マンション購入の流れと手順【2026年版完全ガイド】",
    description: "予算決め→物件探し→申込→審査→契約→引渡しの全ステップを解説。各ステップの期間・費用も網羅。",
  },
};
...
const steps = [
  ...
  },
  {
    ...
    title: "住宅ローン本審査・金消契約",
    period: "2〜4週間",
    color: "border-indigo-300 bg-indigo-500/10",
    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">マンション購入の流れ</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">購入の流れ・手順</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          マンション購入の<span className="text-blue-400">流れと手順</span><br />
        ...
        {/* ━━ 全体スケジュール ━━ */}
        <div className="bg-blue-500/10 border border-blue-200 rounded-2xl p-5 mb-10">
          <p className="text-sm font-black text-blue-200 mb-3">📅 マンション購入にかかる全体期間</p>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center justify-between text-xs">
                ...
                <span className="text-blue-300 font-bold">{item.period}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 pt-3 border-t border-blue-200">
            <p className="text-xs text-blue-200 font-bold">合計：早くて4ヶ月、平均6〜12ヶ月が目安</p>
          </div>
        </div>
          ...
          {steps.map((step, i) => (
            <section key={i} className={`rounded-2xl border-2 p-5 ${step.color}`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-slate-400">{step.period}</span>
              </div>
              <h2 className="text-base font-black text-white mb-3">{step.title}</h2>
              <ul className="space-y-1.5 mb-3">
                {step.points.map((point, j) => (
                  <li key={j} className="text-xs text-slate-200 flex items-start gap-2">
              ...
              {step.tip && (
                <div className="bg-slate-800/70 rounded-xl px-3 py-2">
                  <p className="text-xs text-slate-300">
                    ...
                    {step.tipLink && (
                      <Link href={step.tipLink} className="text-blue-400 hover:underline ml-1">→ 診断ツールで確認</Link>
                    )}
                  </p>
                </div>
              )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  ...
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">目安（3,000万物件）</th>
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">備考</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-100">{row.item}</td>
                ...
                <tr className="bg-gray-100">
                  <td className="px-4 py-3 text-xs font-black text-white">合計目安</td>
                  <td className="px-4 py-3 text-right text-sm font-black text-red-400">約150〜230万円</td>
                  <td className="px-4 py-3 text-xs text-slate-400">中古・仲介あり物件の場合</td>
                </tr>
              </tbody>
            </table>
          </div>
