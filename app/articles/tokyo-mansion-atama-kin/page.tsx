    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">頭金の目安</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">頭金・東京不動産</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          東京でマンション購入に必要な<br />
          <span className="text-blue-400">頭金はいくら</span>？2025年最新データで解説
        </h1>
          ...
          <div className="grid grid-cols-1 gap-3 mb-4">
            {[
              ...
              <div key={i} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                <p className="text-xs text-slate-400 mb-1">{item.area}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">頭金割合</th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">頭金額<br /><span className="text-xs font-normal">(5,000万の場合)</span></th>
                  <th className="text-right px-4 py-3 font-bold text-slate-200 border-b border-slate-700">借入額</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 font-semibold text-white">{row.ratio}</td>
                    <td className="px-4 py-3 text-right text-slate-200">{row.kin === 0 ? "—" : `${row.kin.toLocaleString()}万円`}</td>
                    <td className="px-4 py-3 text-right text-blue-300 font-bold">{row.borrow.toLocaleString()}万円</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-3 mb-4">
            {[
              { borrow: 5000, monthly: 15.3, total: 6416, interest: 1416, bg: "bg-red-500/10 border-red-200" },
              { borrow: 4500, monthly: 13.8, total: 5774, interest: 1274, bg: "bg-yellow-500/10 border-yellow-200" },
              { borrow: 4000, monthly: 12.2, total: 5133, interest: 1133, bg: "bg-green-500/10 border-green-200" },
            ].map((row, i) => (
              <div key={i} className={`rounded-xl p-4 border ${row.bg}`}>
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm font-bold text-slate-100">借入{row.borrow.toLocaleString()}万円</p>
                    <p className="text-xs text-slate-300">月返済 約{row.monthly}万円</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-400">総返済額</p>
                    <p className="text-base font-black text-white">{row.total.toLocaleString()}万円</p>
                    <p className="text-xs text-slate-400">うち利息 {row.interest.toLocaleString()}万円</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section className="mb-10">
          ...
          <div className="bg-blue-500/10 rounded-xl p-5 border border-blue-500/20 mb-4">
            <p className="text-sm font-bold text-blue-200 mb-3">現実的な推奨ライン</p>
            <ul className="text-sm text-slate-200 space-y-2">
              <li className="flex items-start gap-2"><span className="text-emerald-400 font-bold">◎</span><span><strong>物件価格の10〜20%以上</strong>：理想的。月返済額・利息ともに抑えられ、家計の安定性が高い</span></li>
              <li className="flex items-start gap-2"><span className="text-blue-400 font-bold">○</span><span><strong>諸費用分（3〜7%）だけは現金準備</strong>：フルローンを選ぶ場合でも、諸費用分は現金が必要</span></li>
              <li className="flex items-start gap-2"><span className="text-orange-400 font-bold">△</span><span><strong>手元資金をゼロにしない</strong>：頭金に使い切って緊急予備費がなくなるのは危険。生活費3〜6ヶ月分は手元に残す</span></li>
            </ul>
          </div>
          <p className="text-sm text-slate-300 leading-relaxed">
        ...
        <section>
          ...
          <div className="space-y-2">
            ...
            <Link href="/articles/mansion-kounyu-nagare" className="flex items-center gap-3 bg-slate-800 rounded-xl p-3 border border-slate-700 hover:border-blue-500/40 transition-colors group">
              <span className="text-xl">📋</span>
              <span className="text-sm text-slate-200 group-hover:text-blue-400">マンション購入の流れと手順【2026年版・完全ガイド】</span>
            </Link>
          </div>
        </section>
