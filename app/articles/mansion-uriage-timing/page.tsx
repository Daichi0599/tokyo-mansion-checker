...
export const metadata: Metadata = {
  title: "マンションの売り時はいつ？築年数・2026年市況から見る最適タイミング｜30Lab",
  description:
    "マンションの価格下落カーブと2026年の売却市況から、売り時の判断基準を徹底解説。築5年・10年・15年別の価格推移、「売り先行」vs「買い先行」のリスク比較、売却益を最大化するポイントをまとめました。",
  keywords: [
    "マンション 売却 タイミング",
    "マンション 売り時",
    "マンション 築年数 売却",
  ...
  openGraph: {
    title: "マンションの売り時はいつ？築年数・2026年市況から見る最適タイミング",
    description: "価格下落カーブと2026年市況から売り時を解説。築5〜15年別の価格推移と売却益最大化のポイント。",
  },
};
    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-500 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">マンションの売り時</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">売却・資産運用</span>
          <span className="text-xs text-slate-500">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          マンションの<span className="text-blue-400">売り時はいつ？</span><br />
          築年数・市況から見る最適タイミング
        </h1>
        ...
        <section className="mb-10">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">築年数</th>
                  <th className="text-center px-4 py-3 font-bold text-blue-300 border-b border-slate-700">価格水準（目安）</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-200">{row.nensu}</td>
                    <td className="px-4 py-3 text-center text-xs font-bold text-blue-300">{row.price}</td>
          ...
          <p className="text-xs text-slate-500">※東京都内のマンション取引データをもとにした概算値。エリア・グレードにより大きく異なります。</p>
        </section>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                <p className="text-sm font-bold text-slate-100 mb-1">{item.icon} {item.title}</p>
                <p className="text-xs text-slate-300 leading-relaxed">{item.body}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className={`rounded-xl p-4 border ${item.bg}`}>
                ...
                <p className="text-xs text-blue-300 font-bold mb-2">{item.icon}</p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <p className="text-xs font-bold text-blue-400 mb-1">メリット</p>
                    {item.pros.map((p, j) => (
                      <p key={j} className="text-xs text-slate-300 flex items-start gap-1"><span className="text-green-500">✓</span>{p}</p>
                    ))}
                  </div>
                  <div>
                    <p className="text-xs font-bold text-red-400 mb-1">デメリット</p>
                    {item.cons.map((c, j) => (
                      <p key={j} className="text-xs text-slate-300 flex items-start gap-1"><span className="text-red-400">✗</span>{c}</p>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="space-y-2">
            ...
            <Link href="/articles/mansion-floor-plan" className="flex items-center gap-3 bg-slate-800 rounded-xl p-3 border border-slate-700 hover:border-blue-500/40 transition-colors group">
              <span className="text-xl">📐</span>
              <span className="text-sm text-slate-200 group-hover:text-blue-400">マンションの間取り選び方｜1LDK・2LDK・3LDKの違いと資産価値</span>
            </Link>
          </div>
        </section>
