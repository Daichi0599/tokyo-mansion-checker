    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <nav className="text-xs text-slate-400 mb-6 flex items-center gap-1">
          ...
          <span>/</span>
          <span className="text-slate-300">マンションの間取り選び方</span>
        </nav>

        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-500/10 text-blue-300 font-semibold px-2 py-0.5 rounded-full">間取り・物件選び</span>
          <span className="text-xs text-slate-400">2025年最新</span>
        </div>

        <h1 className="text-2xl font-black text-white leading-tight mb-4">
          マンションの<span className="text-blue-400">間取り選び方</span>｜<br />
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-3 py-3 font-bold text-slate-200 border-b border-slate-700">ライフステージ</th>
                  <th className="text-center px-3 py-3 font-bold text-blue-300 border-b border-slate-700">おすすめ間取り</th>
                  <th className="text-left px-3 py-3 font-bold text-slate-200 border-b border-slate-700">理由・ポイント</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-3 py-2 font-semibold text-slate-200">{row.stage}</td>
                    <td className="px-3 py-2 text-center font-bold text-blue-300">{row.madori}</td>
        ...
        <section className="mb-10">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-700 mb-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left px-4 py-3 font-bold text-slate-200 border-b border-slate-700">間取り</th>
                  <th className="text-center px-4 py-3 font-bold text-blue-300 border-b border-slate-700">広さ目安</th>
                  <th className="text-center px-4 py-3 font-bold text-emerald-400 border-b border-slate-700">賃料相場/月</th>
                  <th className="text-center px-4 py-3 font-bold text-orange-400 border-b border-slate-700">主な需要層</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-800" : "bg-slate-700/30"}>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-200">{row.madori}</td>
                    <td className="px-4 py-3 text-center text-xs text-blue-300">{row.hirosa}</td>
                    <td className="px-4 py-3 text-center text-xs font-bold text-emerald-400">{row.chinryo}</td>
                    <td className="px-4 py-3 text-center text-xs text-orange-400">{row.juyo}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-400">※2025年時点の東京23区の参考相場。エリア・グレード・築年数により大きく異なります。</p>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20 mb-4">
            <p className="text-sm font-bold text-blue-200 mb-3">リセールに強い間取りの優先順位</p>
            <ol className="space-y-2">
              {[
                ...
                <li key={i} className="flex items-start gap-2">
                  <span className="font-black text-blue-400 text-sm">{i + 1}.</span>
          ...
          <div className="bg-yellow-500/10 rounded-xl p-4 border border-yellow-200">
            <p className="text-xs font-bold text-yellow-300 mb-1">⚠️ 「自分に合った間取り」と「売りやすい間取り」のバランス</p>
            <p className="text-xs text-slate-300">すべての条件を満たす物件は予算オーバーになりがちです。まずは自分のライフスタイルに合った広さ・部屋数を最優先にし、その中で南向き・駅近・整形などの付加条件を検討しましょう。</p>
          </div>
        </section>
