    ...
    <main className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-block bg-blue-500/15 text-blue-300 text-xs font-bold px-3 py-1 rounded-full border border-blue-500/25 mb-3">
            🏦 無料シミュレーター
          </div>
        ...
        {/* 入力フォーム */}
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-5 mb-6 space-y-6">

          {/* 借入額 */}
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-sm font-bold text-slate-300">借入額</label>
              <span className="text-xl font-black text-blue-400">{fmt(loanMan)}万円</span>
            </div>
            <input
              type="range"
              min={500}
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-full border transition-colors ${
                    loanMan === v
                      ...
                      : "border-slate-600 text-slate-400 hover:border-blue-500 hover:text-slate-200"
                  }`}
                >
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-sm font-bold text-slate-300">年利（金利）</label>
              <span className="text-xl font-black text-blue-400">{rate.toFixed(2)}%</span>
            </div>
            <input
              type="range"
              min={0.1}
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-sm font-bold text-slate-300">返済期間</label>
              <span className="text-xl font-black text-blue-400">{years}年</span>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {scenarios.map((s, i) => (
                  <tr
                    key={i}
                    className={`border-b border-slate-700/50 ${i === 0 ? "bg-blue-500/10" : ""}`}
                  >
                    <td className={`py-2 font-bold ${i === 0 ? "text-blue-400" : "text-slate-300"}`}>
                      {s.r.toFixed(2)}%{i === 0 ? " ← 現在" : i === 1 ? " (+0.5%)" : i === 2 ? " (+1.0%)" : " (+2.0%)"}
                    </td>
                    <td className={`text-right py-2 font-bold ${i === 0 ? "text-blue-400" : "text-slate-200"}`}>
                      {fmtMan(s.monthly)}/月
                    </td>
                    <td className="text-right py-2 text-slate-400">{fmtMan(s.total)}</td>
                    <td className="text-right py-2 text-red-400">{fmtMan(s.interest)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* モゲチェックCTA */}
        <div className="border border-amber-500/40 bg-amber-500/10 rounded-2xl p-5 mb-6">
          <p className="text-sm font-black text-amber-300 mb-1">💡 最安金利で借りると、利息が大幅に減ります</p>
          ...
          <a
            ...
            className="block text-center bg-amber-500 hover:bg-amber-400 text-white font-black text-sm px-6 py-3 rounded-xl transition-colors"
          >
            ✨ 無料でローン金利を一括比較する →
          </a>
        ...
        {/* 診断ツールCTA */}
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 text-center mb-8">
          <p className="text-base font-black text-white mb-1">「安全に買える価格」も確認しませんか？</p>
          <p className="text-xs mb-4 text-blue-200">年収・頭金・生活費から、無理のない購入価格を診断します。</p>
          <Link
            href="/mansion"
